...
注意：在通过上位机修改电机 ID 时，建议按照以下顺序操作：

1. 使用电机排线连接电机和控制板。
2. 使用电源给控制板供电。
3. 使用 Type-C USB 线连接控制板和 PC，然后打开上位机修改 ID。
4. 修改完成后，依次关闭上位机、拔下 USB、断开控制板电源，再更换下一个电机。
...
> **提示：** 装配前建议先阅读本页末尾的完整接线说明。机械臂整体采用串联接线方式，需要在装配过程中同步连接对应排线。
...
5. 最后末端夹爪的装配，有两个注意点
    ...
    - **先将平行夹爪手指与导轨配合后**，再放到外壳上固定；然后先将螺母卡入底部凹槽，再将螺丝打入对应位置，使导轨与外壳固定。
<div align="center">
   <img
